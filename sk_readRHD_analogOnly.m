...
function intanData = sk_readRHD_analogOnly(filename)
%% sk_readRHD_analogOnly Reads only the data from analog channels in RHD files
% INPUTS:
...
if (data_present)
    ...
    t_amplifier = zeros(1, num_amplifier_samples,'int32');

    amplifier_data = zeros(num_amplifier_channels, num_amplifier_samples,'int32');
    aux_input_data = zeros(num_aux_input_channels, num_aux_input_samples,'int32');
...
if (data_present)
    ...
    amplifier_data = int16(amplifier_data - int32(32768)); % units = microvolts
...
intanData = struct('amplifier_data',amplifier_data,...
    'scaleFactor',scaleFactor,'t_amplifier',t_amplifier,...
    'board_adc_data',board_adc_data,'sample_rate',sample_rate,...
